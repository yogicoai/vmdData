...
// ISO(YYYY-MM-DD) → "2026. 5. 29(금)". 이미 자유텍스트면 그대로.
const kdate = (v) => {
  if (!v) return '';
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(v);
  if (!m) return v;
  const d = new Date(Number(m[1]), Number(m[2]) - 1, Number(m[3]));
  return `${m[1]}. ${Number(m[2])}. ${Number(m[3])}(${DOW[d.getDay()]})`;
};
...
export async function exportPromoXlsx(order, cfg) {
  ...
  data.push([title]);
  data.push([]);
  data.push(['1. 발주요청일', '', kdate(order.orderDate)]);
  data.push(['2. 매장도착요청일', '', kdate(order.arriveDate)]);
